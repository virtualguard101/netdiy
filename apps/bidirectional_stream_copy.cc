...
void bidirectional_stream_copy(Socket& socket, string_view peer_name) {
  ...
  eventloop.add_rule(
      "read from stdin into outbound byte stream", input, Direction::In,
      [&] {
        string data;
      ...
      [&] {
        return !outbound.has_error() and !inbound.has_error() and
               (outbound.writer().available_capacity() > 0) and
               !outbound.writer().is_closed();
      },
      [&] { outbound.writer().close(); },
      [&] {
  ...
  eventloop.add_rule(
      ...
      [&] {
        return outbound.reader().bytes_buffered() or
               (outbound.reader().is_finished() and not outbound_shutdown);
      },
      [&] { outbound.writer().close(); },
      [&] {
  ...
  eventloop.add_rule(
      ...
      [&] {
        return !inbound.has_error() and !outbound.has_error() and
               (inbound.writer().available_capacity() > 0) and
               !inbound.writer().is_closed();
      },
      [&] { inbound.writer().close(); },
      [&] {
  ...
  eventloop.add_rule(
      "read from inbound byte stream into stdout", output, Direction::Out,
      [&] {
      ...
      [&] {
        return inbound.reader().bytes_buffered() or
               (inbound.reader().is_finished() and not inbound_shutdown);
      },
      [&] { inbound.writer().close(); },
      [&] {
