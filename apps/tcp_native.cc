...
void show_usage(const char* argv0) {
  cerr << "Usage: " << argv0 << " [-l] <host> <port>\n\n"
       << "  -l specifies listen mode; <host>:<port> is the listening "
          "address.\n";
}
  ...
  try {
    ...
    auto socket = [&] {
      if (server_mode) {
        ...
        cerr << "DEBUG: New connection from "
             << connected_socket.peer_address().to_string() << ".\n";
      ...
      cerr << "DEBUG: Successfully connected to "
           << connecting_socket.peer_address().to_string() << ".\n";
