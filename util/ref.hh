 ...
 * Whether "borrowed" or "owned", the Ref exposes a constant reference to the
 * inner T. If "owned", the inner T can also be accessed by non-const reference
 * (and mutated).
 */
template <typename T>
class Ref {
 ...
 public:
  ...
  // implicit copy via copy constructor -> owned reference (copied from
  // original)
