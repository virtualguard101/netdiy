...
struct BytesPending : public ExpectNumber<Reassembler, uint64_t> {
  ...
  uint64_t value(const Reassembler& r) const override {
    return r.count_bytes_pending();
  }
};
...
struct Insert : public Action<Reassembler> {
  ...
  void execute(Reassembler& r) const override {
    r.insert(first_index_, data_, is_last_substring_);
  }
};
