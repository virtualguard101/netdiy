...
void speed_test(
  ...
  for (size_t i = 0; i < data.size(); i += capacity) {
    ...
    while (true) {
      split_data.emplace(chunk_begin, data.substr(chunk_begin, chunk_size),
                         chunk_begin + chunk_size >= data.size());
      if (chunk_begin >= overlap) {
        chunk_begin -= overlap;
      } else {
        split_data.emplace(0, data.substr(0, chunk_size),
                           0 + chunk_size >= data.size());
        break;
      }
    }
  }
  ...
  while (not split_data.empty()) {
    ...
    while (reassembler.reader().bytes_buffered()) {
      ...
      reassembler.reader().pop(output_data.size() -
                               reassembler.reader().bytes_popped());
    }
  }
  ...
  debug_output << "        Reassembler throughput " << scenario << fixed
               << setprecision(2) << setw(5) << gigabits_per_second
               << " Gbit/s\n";
