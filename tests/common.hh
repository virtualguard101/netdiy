...
class ExpectationViolation : public std::runtime_error {
 public:
  ...
  inline ExpectationViolation(const std::string& property_name,
      ...
      : ExpectationViolation{"should have had " + property_name + " = " +
                             to_string(expected) + ", but instead it was " +
                             to_string(actual)} {}
};
...
struct TestStep {
  ...
  virtual constexpr std::string obj() const {
...
class TestHarness {
  struct DebugHandler {
    explicit DebugHandler(TestHarness* harness) {
      set_debug_handler(debug_in_test<T>, harness);
    }
 ...
 protected:
  explicit TestHarness(std::string test_name, std::string_view desc, T&& object)
    ...
    finish_step("Initialized " + demangle(typeid(T).name()) + " with " +
                    std::string{desc},
                Printer::def);
  }
 ...
 public:
  ...
  void debug(std::string_view message) {
    ...
    if (debug_output_.size() > 1000) {
      throw TestException{
          "the individual test step wrote more than 1,000 debug messages"};
    }
  }
...
template <typename T>
void debug_in_test(void* harness, std::string_view message) {
  reinterpret_cast<TestHarness<T>*>(harness)->debug(
      message);  // NOLINT(*-reinterpret-cast)
}

template <class T>
struct Expectation : public TestStep<T> {
  constexpr std::string str() const override {
    return this->obj() + " expectation: " + description();
  }
...
struct Action : public TestStep<T> {
  constexpr std::string str() const override {
    return this->obj() + " action: " + description();
  }
  virtual std::string description() const = 0;
