...
struct Push : public Action<ByteStream> {
  ...
  std::string description() const override {
    return "push \"" + pretty_print(data_) + "\" to the stream";
  }
...
struct Peek : public Expectation<ByteStream> {
  ...
  void execute(const ByteStream& bs) const override {
    ...
    while (auto bytes_buffered = local_copy.reader().bytes_buffered()) {
      ...
      if (bytes_buffered <= bytes_buffered_after) {
        throw ExpectationViolation{
            "pop() method didn't decrease bytes_buffered()"};
      }
      if (bytes_buffered - bytes_buffered_after != peeked.size()) {
        throw ExpectationViolation{
            "pop(" + to_string(peeked.size()) +
            ") call didn't decrease bytes_buffered() by the correct amount (it "
            "decreased by " +
    ...
    if (got != output_) {
      throw ExpectationViolation{"should have had \"" + pretty_print(output_) +
                                 "\" in buffer, " + "but found \"" +
                                 pretty_print(got) + "\""};
    }
  }
...
struct PeekOnce : public Peek {
  ...
  std::string description() const override {
    return "peek() gives exactly \"" + pretty_print(output_) + "\"";
  }
  ...
  void execute(const ByteStream& bs) const override {
    ...
    if (peeked != output_) {
      throw ExpectationViolation{
          "peek() should have returned \"" + pretty_print(output_) +
...
struct IsClosed : public ExpectBool<ByteStream> {
  ...
  bool value(const ByteStream& bs) const override {
...
struct BytesBuffered : public ExpectNumber<ByteStream, uint64_t> {
  ...
  size_t value(const ByteStream& bs) const override {
...
struct BufferEmpty : public ExpectBool<ByteStream> {
  ...
  std::string name() const override {
    return "bytes_buffered() == 0 [buffer empty]";
  }
  bool value(const ByteStream& bs) const override {
...
struct ReadAll : public Action<ByteStream> {
  ...
  std::string description() const override {
    ...
    return "read \"" + pretty_print(output_) +
           "\" and expect empty buffer after";
  }
  ...
  void execute(ByteStream& bs) const override {
    ...
    if (got != output_) {
      throw ExpectationViolation{"should have read \"" + pretty_print(output_) +
