...
void Printer::print_debug_messages(const DisplayStep& step) const {
  ...
  for (const auto& msg : step.debug_output) {
    if (debug_id == 'a' and repeat_count == 1) {
      cerr << with_color(
          italic, with_color(faint, with_color(step.color,
                                               "    \t  with debug output: ")));
    } else {
      cerr << "    \t                     ";
    }
    string out = (step.debug_output.size() > 1)
                     ? string(repeat_count, debug_id) + ") "
                     : "";
...
void Printer::diagnostic(string_view test_name,
  ...
  cerr << with_color(red,
                     "  ***** Unsuccessful " + failing_step.text + " *****\n");
  print_debug_messages(failing_step);
  cerr << "\n";

  cerr << with_color(red, exception_type) << ": "
       << with_color(def, exception_message) << "\n\n";
}
...
Timeout::Timer::Timer() {
  static constexpr itimerval deadline{.it_interval = {0, 0},
                                      .it_value = {2, 0}};
...
void throw_timeout(int signal_number) {
  ...
  throw TestException{
      "the individual test step took longer than 2 seconds (possibly an "
      "infinite loop?)"};
}
...
Timeout::Timer
Timeout::make_timer()  // NOLINT(readability-convert-member-functions-to-static)
{
  return {};
}
