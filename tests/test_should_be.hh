...
inline std::string human_compare(const Wrap32& actual, const Wrap32& expected) {
  std::ostringstream ss;
  uint32_t actual_raw =
      minnow_conversions::DebugWrap32{actual}.debug_get_raw_value();
  uint32_t expected_raw =
      minnow_conversions::DebugWrap32{expected}.debug_get_raw_value();
  uint32_t diff_high = actual_raw - expected_raw;
  uint32_t diff_low = expected_raw - actual_raw;
  if (diff_high < diff_low) {
    ss << "The Wrap32's raw value was too high by " << diff_high
       << human(diff_high) << ".\n";
  } else {
    ss << "The Wrap32's raw value was too low by " << diff_low
       << human(diff_low) << ".\n";
  }
  return ss.str();
}
...
static void test_should_be_helper(const T& actual, const T& expected,
  ...
  if (actual != expected) {
    std::ostringstream ss;
    ss << "\nThe expression `" << actual_s << "` evaluated to "
       << to_string(actual) << ",\nbut was expected to equal `" << expected_s
       << "` (which evaluates to " << to_string(expected) << ").\n\n";
